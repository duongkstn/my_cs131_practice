...
def compute_distances(X1, X2):
    ...
    x12 = -2 * np.dot(X1, X2.T)
    sqx1 = np.sum(X1 ** 2, axis = 1).reshape(M, 1)
    sqx2 = np.sum(X2 ** 2, axis = 1).reshape(1, N)
...
def split_folds(X_train, y_train, num_folds):
    ...
    dai = X_train.shape[0]
    kc = int(dai / num_folds)
    j = 0
    for i in range(0,X_train.shape[0], kc):
        if i + kc - 1 < dai:
            X_vals[j] = X_train[i: i + kc]
            y_vals[j] = y_train[i: i + kc]
            X_trains[j] = np.concatenate([X_train[:i],X_train[i + kc:]])
            y_trains[j] = np.concatenate([y_train[:i],y_train[i + kc:]])
            j += 1  
    '''
