...
class PCA(object):
    ...
    def fit(self, X, method='svd'):
        ...
        if method == 'eigen':
            self.W_pca, _ = self._eigen_decomp(X_centered) #actually W_pca is array of eig vectors !!
        else:
    ...
    def _eigen_decomp(self, X):
        ...
        sort_perm = e_vals.argsort()[::-1]

        e_vals = e_vals[sort_perm]     # <-- This sorts the list in place.
        e_vecs = e_vecs[:,sort_perm]
    ...
    def _svd(self, X):
        ...
        #the rows (not columns) !!!!! of  VT  are the eigenvectors of  XTX!!!!!!
        vecs = vh.T
        vals = s #vals is not true e_vals
    ...
    def reconstruct(self, X_proj):
        ...
        D = self.W_pca.shape[0]
        X = X_proj.dot(np.linalg.inv(self.W_pca)[:n_components,:])
...
class LDA(object):
    ...
    def fit(self, X, y):
        ...
        #Sb * wi = lambda_i * Sw * wi
        #==> Sw^-1 * Sb *wi = lambda_i * wi
        #==>lambda_i is eigen values of Sw^-1 * Sb
    ...
    def _between_class_scatter(self, X, y):
        ...
        ''' Whay false ? because the equation is not clear
        mu = X.mean(axis=0)
        for i in np.unique(y):
            # YOUR CODE HERE
            X_i = X[y == i]
            mu_i = np.mean(X_i, axis = 0)
            mu_i_centered = mu_i - mu
            S_b = (X_i.shape[0]) * mu_i_centered.T.dot(mu_i_centered)
            print('S_b = ',S_b)
            scatter_between += S_b
            # END YOUR CODE
