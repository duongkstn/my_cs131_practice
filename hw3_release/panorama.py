...
def harris_corners(img, window_size=3, k=0.04):
    ...
    for x in range(H):
        for y in range(W):
            M = np.array([[Ix2[x,y],Ixy[x,y]],[Ixy[x,y],Iy2[x,y]]])
...
def match_descriptors(desc1, desc2, threshold=0.5):
    ...
    dists = cdist(desc1, desc2) #(M,N) matrix

    ### YOUR CODE HERE
    for i in range(M):
        dists_i = dists[i,:]
        tmp = dists_i.argsort()
        if dists_i[tmp[0]] / dists_i[tmp[1]] < threshold:
            
            matches.append((i, tmp[0]))
...
def ransac(keypoints1, keypoints2, matches, n_iters=200, threshold=20):
    ...
    for i in range(n_iters):
        ...
        maybeinliers_index = random.sample([x for x in range(N)], n_samples)
        maybeinliers = matches[maybeinliers_index]
        p1 = np.zeros((N, 2))
        p2 = np.zeros((N, 2))
        for idx,tup in enumerate(maybeinliers):
            ind1 = tup[0]
            ind2 = tup[1]
            p1[idx] = np.array([keypoints1[ind1][0], keypoints1[ind1][1]])
            p2[idx] = np.array([keypoints2[ind2][0], keypoints2[ind2][1]])
        maybemodel = fit_affine_matrix(p1[:n_samples], p2[:n_samples]) 
        alsoinliers_index = []
        for j in range(N):
            p2 = matched2[j]
            p1 = matched1[j]
            p1hat = p2.dot(maybemodel)
            error = np.sum((p1hat - p1) ** 2)
            if error <= threshold:
                alsoinliers_index.append(j)
        if (len(alsoinliers_index) > maxx):
            maxx = len(alsoinliers_index) 
            H = maybemodel
            index_result = alsoinliers_index
    inliers = matches[index_result]
    p1 = np.zeros((maxx, 2))
    p2 = np.zeros((maxx, 2))
    for idx, tup in enumerate(inliers):
        ind1 = tup[0]
        ind2 = tup[1]
        p1[idx] = np.array([keypoints1[ind1][0], keypoints1[ind1][1]])
        p2[idx] = np.array([keypoints2[ind2][0], keypoints2[ind2][1]])
    H = fit_affine_matrix(p1, p2)
    #H = fit_affine_matrix(keypoints1[index_result], keypoints2[index_result])
...
def hog_descriptor(patch, pixels_per_cell=(8,8)):
    ...
    ### YOUR CODE HERE
    gra_hist = np.zeros((rows, cols, n_bins))
    for i in range(rows):
        for j in range(cols):
            theta_bin = np.floor(theta_cells[i,j] / degrees_per_bin).astype(int) #[int 0- 9]
            under = theta_bin * degrees_per_bin
            above = under + degrees_per_bin
            
            for m in range(theta_bin.shape[0]):
                for n in range(theta_bin.shape[1]):
                    g_cell = G_cells[i,j,m,n]
                    if theta_cells[i,j,m,n] == under[m,n]: #[0,20,..,160,180]
                        if theta_bin[m,n] == 9: #180
                            gra_hist[i,j,0] += g_cell
                        else:
                            gra_hist[i,j,theta_bin[m,n]] += g_cell
                    else:
                        gra_hist[i,j,theta_bin[m,n]]+=(theta_cells[i,j,m,n] - under[m,n]) / degrees_per_bin  * g_cell
                        if theta_bin[m,n] + 1 < 9:
                            gra_hist[i,j,theta_bin[m,n]+1]+=(-theta_cells[i,j,m,n] + above[m,n]) / degrees_per_bin * g_cell
                        else:
                            gra_hist[i,j,0] += (-theta_cells[i,j,m,n] + above[m,n]) / degrees_per_bin * g_cell
                              
            gra_hist[i,j] = gra_hist[i,j] / np.linalg.norm(gra_hist[i,j])
    block = gra_hist.flatten()
...
def linear_blend(img1_warped, img2_warped):
    ...
    ### YOUR CODE HERE
    kc = right_margin - left_margin
    #img1_mask[:,:left_margin] = np.tile(np.linspace(1, 1, num = left_margin), (img1_warped.shape[0],1))
    img1_mask[:,left_margin : right_margin] = np.tile(np.linspace(1, 0, num = kc), (img1_warped.shape[0],1))
    img2_mask[:,left_margin : right_margin] = np.tile(np.linspace(0, 1, num = kc), (img2_warped.shape[0],1))
    #img2_mask[:,right_margin:] = np.tile(np.linspace(1, 1, num = img2_mask.shape[1] - right_margin), (img2_warped.shape[0],1))
    img1_warped *= img1_mask
    img2_warped *= img2_mask
...
def stitch_multiple_images(imgs, desc_func=simple_descriptor, patch_size=5):
    ...
    iref = imgs[center]
    H_all = []
    for i in range(len(imgs) - 1): #H[i]: i and i + 1
        h,_ = ransac(keypoints[i], keypoints[i + 1], matches[i], threshold = 1)
        H_all.append(h)
    print(len(H_all)) #3
    print(len(imgs)) #4 [0,1,2,3]
    print(center) #1
    ...
    for i in range(center - 1, -1, -1):
        if i == center - 1:
            H[i] = np.linalg.inv(H_all[i])
        else:
            H[i] = np.dot(H[i + 1], np.linalg.inv(H_all[i]))
    for i in range(center + 1, len(imgs)):
        if i == center + 1:
            H[i] = H_all[i - 1]
        else:
            H[i] = np.dot(H[i - 1], H_all[i - 1])
    H[center] = np.eye(3)
    #print(H)
    
    output_shape, offset = get_output_space(iref, imgs, H)
    panorama = None
    import matplotlib.pyplot as plt
    img_warped = []
    for i in range(len(imgs)):    
        img_i_warped = warp_image(imgs[i], H[i], output_shape, offset)
        img_i_mask = (img_i_warped != -1)
        img_i_warped[~img_i_mask] = 0
        img_warped.append(img_i_warped)
        plt.imshow(img_i_warped)
        plt.axis('off')
        plt.show()
    panorama = img_warped[0]
    for i in range(1, len(img_warped)):
        panorama = linear_blend(panorama, img_warped[i])
    #panorama = sum(img_warped)
    ### END YOUR CODE
