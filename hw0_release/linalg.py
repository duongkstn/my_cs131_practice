...
def complicated_matrix_function(M, a, b):
    ...
    aTb = dot_product(a.T,b)
    MaT = dot_product(M,a.T)
    #out = dot_product(aTb,MaT)
    out = aTb * MaT.reshape(MaT.shape[0],1)
    ### END YOUR CODE
...
def get_eigen_values_and_vectors(M, k):
    ...
    for i in range(m):
        tmp = abs(w[i]), w[i], v[:,i]
        li.append(tmp)
    li = sorted(li, key = FirstElem)[:k]
    for i in range(k):
        eigenvalues.append(li[i][1])
        eigenvectors.append(li[i][2].reshape(m,1))
    ### END YOUR CODE
