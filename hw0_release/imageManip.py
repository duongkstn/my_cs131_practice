...
def dim_image(image):#lam mo anh di
...
def mix_images(image1, image2, channel1, channel2):
    ...
    widthchia2 = int(width / 2)
    out1 = rgb_exclusion(image1, channel1)
    out2 = rgb_exclusion(image2, channel2)
    out = np.concatenate([out1[:,:widthchia2,:],out2[:,widthchia2:,:]], axis = 1)
    ### END YOUR CODE
...
def mix_quadrants(image):
    ...
    h = int(height / 2)
    w  = int(width / 2)
    i1 = rgb_exclusion(image[:h,:w], 'R')
    i2 = dim_image(image[:h,w:])
    i3 = image[h:,:w] ** 0.5
    i4 = rgb_exclusion(image[h:,w:], 'R')
    out = np.copy(image)
    out[:h,:w] = i1
    out[:h,w:] = i2
    out[h:,:w] = i3
