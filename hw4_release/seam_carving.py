...
def compute_cost(image, energy, axis=1):
    ...
    ### YOUR CODE HERE
    for i in range(1, H):
        b = np.roll(cost[i - 1], 1).reshape(1, W)
        b[:,0] = np.max(cost[i - 1]) + 1 
        c = np.roll(cost[i - 1], -1).reshape(1, W)
        c[:,-1] = np.max(cost[i - 1])  + 1
        a = np.concatenate([b, cost[i - 1].reshape(1, W), c], axis = 0)
        
        minn = np.min(a, axis = 0)
        cs = np.argmin(a, axis = 0)
        
        cost[i] = energy[i] + minn
        paths[i] = cs - 1
    ### END YOUR CODE
...
def enlarge(image, size, axis=1, efunc=energy_function, cfunc=compute_cost):
    ...
    w = W
    k = size - W
    seams = find_seams(out, k, axis=1, efunc=efunc, cfunc=cfunc)
    indices = np.tile(range(W), (H, 1))  # shape (H, W)
    for i in range(k):
        seam = np.where(seams == i + 1)[1]
        out = duplicate_seam(out, indices[range(H), seam])
        for j in range(H):
            indices[j, seam[j]:] += 1
...
def compute_forward_cost(image, energy):
    ...
    for i in range(1, H):
        for j in range(W):
            if (j > 0 and j < W-1):
                chung = abs(image[i,j + 1] - image[i,j - 1])
                Cl = chung + abs(image[i-1, j] - image[i,j-1])
                Cr = chung + abs(image[i-1, j] - image[i,j+1])
                Cv = chung
                cost[i,j] = energy[i,j] + min(cost[i-1,j-1] + Cl, cost[i-1,j] + Cv, cost[i-1,j+1] + Cr)
                paths[i,j] = np.argmin([cost[i-1,j-1] + Cl, cost[i-1,j] + Cv, cost[i-1,j+1] + Cr]) - 1
            elif j == 0:
                Cr = abs(image[i-1, j] - image[i,j+1])
                Cv = 0
                cost[i,j] = energy[i,j] + min(cost[i-1,j] + Cv, cost[i-1,j+1] + Cr)
                paths[i,j] = np.argmin([cost[i-1,j] + Cv, cost[i-1,j+1] + Cr])
            else:
                Cl = abs(image[i-1, j] - image[i,j-1])
                Cv = 0
                cost[i,j] = energy[i,j] + min(cost[i-1,j-1] + Cl, cost[i-1,j] + Cv)
                paths[i,j] = np.argmin([cost[i-1,j-1] + Cl, cost[i-1,j] + Cv]) - 1
...
def reduce_fast(image, size, axis=1, efunc=energy_function, cfunc=compute_cost):
    ...
    for i in range(k):
        seam = np.where(seams == i + 1)[1]
        out = remove_seam(out, indices[range(H), seam])
        for j in range(H):
            indices[j, seam[j]:] -= 1
    ### END YOUR CODE
...
def remove_object(image, mask, axis):
    ...
    while True:
        ...
        seam = backtrack_seam(paths, end)
        out = remove_seam(out, seam)
        m = remove_seam(m, seam)
        
        if np.all(m == 0):
            break
    out = enlarge(out, out.shape[1] + num, axis = 1, efunc = efunc, cfunc=compute_cost) #new cfunc
    ### END YOUR CODE
    if axis == 0:
        out = np.transpose(out, (1, 0, 2))
