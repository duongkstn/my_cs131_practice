...
def conv(image, kernel):
    ...
    for m in range(Hi):
        for n in range(Wi):
            #print(m,' ',n,' ',hk,' ',wk)
            out[m,n] = np.sum(padded[m:m + Hk, n:n + Wk] * kernel_flip)
    ### END YOUR CODE
...
def gaussian_kernel(size, sigma):
    ...
    for i in range(size):
        for j in range(size):
            kernel[i,j] = 1 / (np.pi*tmp) * np.exp(-((i - k) ** 2 + (j - k) ** 2) / tmp)
    ### END YOUR CODE
...
def non_maximum_suppression(G, theta):
    ...
    theta[theta == 360] = 0
    ### BEGIN YOUR CODE
    direct ={0:(0,1), 45:(-1,1), 90:(-1,0), 135: (-1,-1), 180: (0,-1), 225: (1,-1), 270: (1,0), 315: (1,1)}
    for i in range(H):
        for j in range(W):
            u = i + direct[theta[i,j]][0]
            v = j + direct[theta[i,j]][1]
            maxx = G[i,j]
            
            if (u in range(H) and v in range(W)):
                maxx = max(maxx, G[u,v])
            neg_angle = 180 + theta[i,j]
            if neg_angle >= 360:
                neg_angle -= 360
            u = i + direct[neg_angle][0]
            v = j + direct[neg_angle][1]
            if (u in range(H) and v in range(W)):
                maxx = max(maxx, G[u,v])
            if G[i,j] == maxx:
                out[i,j] = maxx
...
def hough_transform(img):
    ...
    W, H = img.shape
    diag_len = int(np.ceil(np.sqrt(W * W + H * H))) #because HCN
    ...
    for x,y in zip(xs,ys): #respectively
        for ind,_ in enumerate(thetas):

            p = int(x * cos_t[ind] + y * sin_t[ind])
            accumulator[p + diag_len, ind] += 1
