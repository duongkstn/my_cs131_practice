...
from scipy.ndimage import interpolation as it
import math

def hog_feature(image, pixel_per_cell = 8):
    #https://codelungtung.com/2018/08/06/image-descriptor-histogram-of-oriented-gradients/
    """
    ...
    """
    ### YOUR CODE HERE
    hogFeature, hogImage = hg(image, pixels_per_cell=(pixel_per_cell, pixel_per_cell), visualise=True)
    ### END YOUR CODE
...
def sliding_window(image, base_score, stepSize, windowSize, pixel_per_cell=8):
    ...
    for i in range(0, pad_image.shape[0], stepSize):
        for j in range(0, pad_image.shape[1], stepSize):
            if (not(i + winH < pad_image.shape[0] and j + winW < pad_image.shape[1])):
                continue
            r = i // stepSize
            c = j // stepSize
            
            img = pad_image[i:i + winH,j:j + winW]
            response_map[r][c] = base_score.dot(hog_feature(img, pixel_per_cell = pixel_per_cell)[0])
            if (response_map[r][c] > max_score):
                max_score = response_map[r][c]
                maxr = i
                maxc = j 
...
def pyramid_score(image,base_score, shape, stepSize=20, scale = 0.9, pixel_per_cell = 8):
    ...
    ### YOUR CODE HERE
    for (sc, img) in images:
        score, r, c, res_map = sliding_window(img, base_score, stepSize, shape, pixel_per_cell=pixel_per_cell)
        if score > max_score:
            max_score = score
            maxr = r
            maxc = c
            max_scale = sc
            max_response_map = res_map
    ### END YOUR CODE
...
def compute_displacement(part_centers, face_shape):
    ...
    ### YOUR CODE HERE
    d = (face_shape[0] / 2, face_shape[1] / 2) - part_centers
    mu = np.average(d, 0).astype(int)
    sigma = np.std(d, 0)
    ### END YOUR CODE
    return mu, sigma
...
def shift_heatmap(heatmap, mu):
    ...
    new_heatmap = it.shift(heatmap, mu)
    ### END YOUR CODE
    return new_heatmap
...
def gaussian_heatmap(heatmap_face, heatmaps, sigmas):
    ...
    for i in range(len(sigmas)):
        img = gaussian(heatmaps[i], sigma = sigmas[i])
        heatmap += img
    mx = heatmap[0][0]
    r,c = 0,0
